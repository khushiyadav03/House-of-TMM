...
export default function NewArticle() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If the main endpoint fails with 401, try the debug endpoint as fallback
      if (!response.ok && response.status === 401) {
        console.log('Main endpoint failed with 401, trying debug endpoint...');
        response = await fetch("/api/articles/create-debug", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
