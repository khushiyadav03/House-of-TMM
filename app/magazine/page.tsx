...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/magazines/${params.id}`);
  const data = await response.json();
  const magazine = data;
  ...
  return {
    title: magazine.seo_title || magazine.title,
    description: magazine.seo_description || magazine.description,
    keywords: magazine.seo_keywords || ['magazine', magazine.title],
  };
}
